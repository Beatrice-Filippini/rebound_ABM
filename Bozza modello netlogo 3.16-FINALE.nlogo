...
to import-data
  ...
  [
    ...
    set p-stock-threshold-list lput item 13 row p-stock-threshold-list
  ]
  file-close
end
...
to for-scenario-analysis
  ...
  set sa-memory-bought-products lput bought-products sa-memory-bought-products

  ifelse length bought-prod-sust-list = 0
  [set sa-memory-avg-sustainability lput 0 sa-memory-avg-sustainability]
  [set sa-memory-avg-sustainability lput mean bought-prod-sust-list sa-memory-avg-sustainability]


  if (ticks > ticks-scenario)
  [
    set sa-memory-bought-products but-first sa-memory-bought-products
    set sa-memory-avg-sustainability but-first sa-memory-avg-sustainability
  ]
...
@#$#@#$#@
<experiments>
  <experiment name="exp 1 num" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="250"/>
  ...
  <experiment name="exp 2 num" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="250"/>
    <metric>mean sa-memory-bought-products</metric>
    ...
    <enumeratedValueSet variable="file-name-m1">
      ...
      <value value="&quot;Reprocessing_matrix_2.csv&quot;"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="exp 3 num" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="250"/>
    <metric>mean sa-memory-bought-products</metric>
  ...
  <experiment name="Caso base no reprocessing" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>; 1. refresh variables used to make counts
  refresh-variables
  ; 2. the consumer
  user-stock-consumption
  get-normalized-status
  ...
  <experiment name="Case study" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="250"/>
    <metric>mean sa-memory-bought-products</metric>
