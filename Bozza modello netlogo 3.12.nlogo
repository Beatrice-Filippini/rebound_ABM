...
globals [
  ...
  ;this global variable is used for a plot
  chosen-prod-sust-list

  ;used in the strategy-reprocess procedure
  p-sust-increase
  g-total-revenues-list
  c-strategy-frequency
...
to init-globals
  ...
  set c-strategy-frequency 3                                    ;it is the frequency with which the company is authorized to activate the sustainability strategy
...
;        print "------------------------------------------------------------------"
        ...
        set p-production-cost item i p-prod-cost-min-list * (1 + delta * p-sustainability + omega * p-quality)         ;p-price-min represents the production cost baseline
...
;      ]
;      [
;        set p-utility p-utility-init * (1 - i-stock  / i-stock-threshold)
;      ]
...
;                print ([p-sustainability] of matching-products)
              ...
              set p-production-cost item k p-prod-cost-min-list * (1 + delta * p-sustainability + omega * p-quality)
  ...
  ask companies [
    ...
    while [ j < n-class-of-products ] [
      ...
      hatch-products c-production-j [
        ...
        set p-production-cost item j p-prod-cost-min-list * (1 + delta * p-sustainability + omega * p-quality)
...
"" ""
PENS
"default" 1.0 0 -14070903 true "" "plot count products with [owner-id = 1]"
"pen-1" 1.0 0 -5825686 true "" "plot count products with [owner-id = 2]"
"pen-2" 1.0 0 -817084 true "" "plot count products with [owner-id = 3]"
...
"" ""
PENS
"all products" 1.0 0 -16777216 true "" "plot mean [p-sustainability] of products "
"c-sustainability" 1.0 0 -14454117 true "" "plot mean [c-sustainability] of companies"
"reprocessed" 1.0 0 -14439633 true "" "if (any? products with [primary-prod = 0]) [\nplot mean [p-sustainability] of (products with [primary-prod = 0])]"
"primary" 1.0 0 -2674135 true "" "if (any? products with [primary-prod = 1]) [\nplot mean [p-sustainability] of (products with [primary-prod = 1])]"
...
985
505
N° of reprocessed prod per company
NIL
NIL
0.0
10.0
...
"" ""
PENS
"default" 1.0 0 -13345367 true "" "plot count products with [owner-ID = 1 and primary-prod = 0] "
"pen-1" 1.0 0 -8630108 true "" "plot count products with [owner-ID = 2 and primary-prod = 0] "
"pen-2" 1.0 0 -955883 true "" "plot count products with [owner-ID = 3 and primary-prod = 0] "
...
- controllare per bene funzione di acquisto
- valutare se  tornare di nuovo a matrice con hvr, svr, lvr
...
- cercare di sistemare il modo in cui viene settata p-sustainability perché ora non riesce a vedersi l'incremento di sustainability quando quando aumenta la
	- c-sustainability
	- numero riprocessati
- da rivedere per capire se è giusta:
	- p-sustainability: incremento del 10% quando riprocessiamo
	- settata sulla base di prodotti circolanti quando si genera prodotti durante 		la simulazione
	 
- ricordiamoci che abbiamo cambiato di nuovo la funzione di utilità mettendo esponente e togliendo epsilon: valutare a mente fresca se ha senso!!!


## Problemi da risolvere

#### problemi  minori
- capire come gestire meglio la strategia di sostenibilità: 
	- company con revenue max-> non attiva mai la strategia
...
-!!!!! ipotizziamo che, con il passare del tempo, il peso medio che l'utente da alla sostenibilità di un prodotto aumenti lentamente (prevedendo un incremento per ogni tick inversamente proporzionale al numero di tick della simulazione considerata)
...
- in fase di normalizzazione delle variabili usate per calcolare l'utilità, se il valore min e il valore max sono uguali, allora settiamo il valore normalizzato a 0.5
...
- Non viene considerato il costo di smaltimento e di riprocessamento
