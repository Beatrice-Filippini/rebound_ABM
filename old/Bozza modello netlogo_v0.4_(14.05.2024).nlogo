...
create-users n-users[
    ...
    set gamma mean-gamma
    set sustainability-weight random-float 1
    set price-weight random-float 1
  ...
  create-companies n-companies [
    ...
    set product-sustainability random-float 1
    set product-price random 10 ;FIX
    ;set production-capability random 100
    set profit product-price * company-demand
    set target-demand random target-demand-max
    set product-cost 1 ;for now, the production cost is the same for each company
...
to utility-function-management
  ...
  let max-sustainability max [ product-sustainability ] of companies
  let min-sustainability min [ product-sustainability ] of companies
  let max-price max [ product-price ] of companies
  let min-price min [ product-price ] of companies

  ; compute for each company the normalized score for price and sustainabilty
  ask companies [
    set product-sustainability-norm (product-sustainability - min-sustainability) / (max-sustainability - min-sustainability)
    set product-price-norm (product-price - min-price) / (max-price - min-price)
  ]

  ; find the best company according to each user preference
  ask users [

    let my-sustainability-weight sustainability-weight ;alpha
    let my-price-weight price-weight ;beta

    set best-company max-one-of companies [ product-sustainability-norm * my-sustainability-weight - product-price-norm * my-price-weight ]
    set utility [ product-sustainability-norm ] of best-company * my-sustainability-weight - [ product-price-norm ] of best-company * my-price-weight
  ]



  ;set utility-function (sustainability-weight * sustainability-i ^ n) - (price-weight * price-i ^ n)
...
;    print utility * gamma
;    print utility
    ifelse stock / stock-threshold <= utility * gamma [
    ...
    ask companies [
      ...
      set profit company-demand * product-price - product-cost
    ]
  ]
...
;    ifelse ratio < max utility * gamma [
;      set stock-input 1 ;
;    ]
;    [
;      set stock-input 0
  ...
  ask companies[
    ...
    if ticks >= price-period [
        ...
        ; sublist starting from position lenght-3 (inclusive) and lenght (CHECK: guide  says right extremity is exclusive)
                          ...
                          ;;; if demand < target for 3 periods--> decrease price of 10%
        ...
        set bool-decrease-price False
                          ...
                          ;;; if demand > target for 3 periods--> increase price of 10%
        if (company-demand > target-demand) and (mean last-price-period > target-demand)[
...
to sustainability-demand-regulation
  ask companies[
    ...
    if ticks >= sust-period [
        ...
        if(product-sustainability * 1.1 <= 1) [
          set product-sustainability product-sustainability * 1.1  ;increase sustainability of the product of 10%
...
;      if (company-demand > target-demand) and (mean last-sust-period > target-demand)[
;          set product-sustainability product-sustainability * 0.9
